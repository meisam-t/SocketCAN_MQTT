...
int can_init()
{
    ...
    if ((s_id = socket(PF_CAN, SOCK_RAW, CAN_RAW)) < 0)
    {
    ...
    if (bind(s_id, (struct sockaddr *)&addr, sizeof(addr)) < 0)
    {
...
// always share ["270", "27F", "4A0"]
    ...
    setsockopt(s_id, SOL_CAN_RAW, CAN_RAW_FILTER, &filterID, sizeof(filterID));
   //  printf("%d\n",s);
...
//     printf("%s","Inside can recieve ");
	...
	struct can_frame frame;
	int nfd;
    nbytes = read(s_id, &frame, sizeof(struct can_frame));
...
void cansendStruct(struct candata data){
	...
	frame.can_dlc=8;
	frame.data[0]=data.data1;
	frame.data[1]=data.data2;
	frame.data[2]=data.data3;
	frame.data[3]=data.data4;
	frame.data[4]=data.data5;
	frame.data[5]=data.data6;
	frame.data[6]=data.data7;
	frame.data[7]=data.data8;
	write(s_id,&frame,sizeof(struct can_frame));
}*/
void can_send(int id, int serialnum,unsigned char *value){
  	int nbytes;
	struct can_frame frame;
